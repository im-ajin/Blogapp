...
export const getposts = async (req, res, next) => {
    try{
       ...
       const posts = await Post.find(objj).sort({ updatedAt: sortDirection }).skip(startIndex).limit(limit);
...
export const deletepost = async (req, res, next) => {
    if(!req.user.isAdmin || req.user.id !== req.params.userId){
        return next(errorHandler(403, 'You not allowed to delete this post'))
    }
    try{
